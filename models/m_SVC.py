'''
 using support vector machine with rbf kernel from sklearn
'''
import os
import sys
...
def run_m_SVC():
    ...
    MDL = get_m_SVC()
    MDL = CalibratedClassifierCV(MDL)
    #training time
    t_start = time.time()
    ...
    for i in xrange(len(myDS.p35_list)):
        ...
        cname_to_perfs[prj_name] = ['SVM', t_time, cur_accuracy, cur_error, cur_precision, cur_recall, cur_f1, cur_auroc]
        
    with open(PATH_VARIABLES.cv_5fold_repo_SV, 'a') as f:
